...
module.exports = (app, validator) => {
    ...
    app.get('/api/order/:orderId', loggedCheckMiddleware, validator.validate('get', '/order/{orderId}'), (req, res, next) => {
        model.Order.getOrder(req.params.orderId)
            .then(order => res.status(200).json(order.toExpandedJSON()), next)
    });

    app.post('/api/order', loggedCheckMiddleware, validator.validate('post', '/order'), (req, res, next) => {
